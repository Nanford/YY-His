...
- 中医体质 **30 题转化分**（≥40 是 / 30–39 倾向是）；**「倾向是/基本是视同正式标签进推荐」口径废止**（报告仍可标注倾向字样）。
...
- **M5 V2.0 更新**（需求更新说明_V2.0_20260719.md）：积分排名推荐（黄金用例 §4.3：衰弱+存在营养不良风险+跌倒风险筛查阳性+气虚质+血瘀质 → M06/M12/D03/D06/C01/C08 六项，分值 7/7/9/7/9/4）、运动视频与膳食/食养图片教程展示、患者补充评估与历史报告、语音链路时间戳埋点与固定浮层提示。
...
- V2.0 干预素材：D01-D10/C01-C08 图片由 convert-rules 从 docs/source **palette 量化压缩**（目标单张 <600KB，quality 90→50 逐级下调至达标，超出告警不中止）后拷贝到 `public/interventions/`，并同步派生同名 **WebP**（前端 `<picture>` 优先取 WebP、回退 PNG）；图片/视频 `mediaSrc` 均附内容哈希 `?v=`，`/interventions/*` 走 immutable 长缓存（带宽受限服务器减负）；**M01-M12 视频已就位（2026-07-20，按 `<编码>.mp4` 命名放入 `public/interventions/videos/` 并重跑 convert-rules，`mediaAvailable=true`、URL 附内容哈希）**——运动卡片直接播放视频（16:9 卡内播放），播放失败仍回退动作文字要点；后续替换视频文件后同样**重跑 `npm run convert-rules`** 刷新哈希即可，无需改代码。
