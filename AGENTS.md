...
**核心目标**：系统代替医护人员完成老年患者健康信息采集（数字医生语音对话），自动完成标准化评估并生成评估标签，再按积分排名推荐个体化干预方案。Demo 阶段的唯一目标是跑通完整流程：**基础信息录入 → 智能化信息采集 → 采集信息分析和评估 → 干预方案推荐**。

**使用者**：医生（录入、干预方案审核确认）+ 老年患者（大屏语音交互，**可自助建档并直接开始评估，不必等医生先录入**；问答完成后可在同一大屏直接查看评估报告与候选干预方案，无需等待医生前置确认）。两端目前均为独立入口（`/doctor`、`/patient`），Demo 阶段不做真实登录鉴权（2026-07-14 已与用户确认，正式版再加）。患者自助建档只收姓名/性别/年龄（必填）+ 测量数据（选填），并可自选评估内容（四量表多选，默认勾 FRAIL+跌倒；2026-07-15 修订，覆盖当日早先"固定 FRAIL+跌倒"的锁定口径）——含舌象/测量题的量表（MNA-SF/中医体质）在选项上如实提示"这些题暂不计分，先出部分计分报告"：Demo 口径（2026-07-20 拍板）患者答完一律出报告，医生检查题按 `deferClinical` 豁免计分并在报告标注"部分计分"，不再落 `awaiting_doctor`（仅普通问答题"待人工确认"未补录时才需医生补录）；医生仍可在 `/doctor/patients/new` 录入完整档案（含身份证/手机/住址/住院号/门诊号）并勾选任意量表——两条入口共用同一套校验规则（`src/lib/assessment/patient-intake.ts`），产生的患者/会话完全同构，医生审核干预方案这一关不因入口不同而跳过。

**当前阶段**：Demo / 探索。演示优先，但评分引擎、数据模型按正式产品标准做（后续会持续增加量表、标签和映射）。

**V2 迭代进行中（2026-08-06 启动）**：需求与规则源文件在 `V2/`（4 张 xlsx + Demo_v2更新说明.docx + figure 1.png，只读），进度事实来源是 **`docs/V2-迭代任务清单.md`**——任何会话开工前先读它定位断点（找第一个未完成项 + 看"当前断点"节），完成一项就勾选并更新断点。V2 关键口径变化（中医体质转化分、推荐 100/-100 语义、5 大类 60 项干预、倾向是/基本是推荐口径废止等）以任务清单"关键口径"节为准，与本文件旧口径冲突处在 M11 统一改写。
...
**医学口径（用户已拍板）**：体质"倾向是"（9～10 分）与平和质"基本是"**触发干预推荐**，视同正式体质标签按积分矩阵全分参与累加；评估报告与积分明细中仍标注"（倾向）"字样以示区分。
...
5. **干预方案必须展示正文与素材**：不能只显示干预名称，必须同时展示完整教程内容（V2.0：运动项为视频教程、缺失时回退《12种运动干预.docx》动作文字要点；膳食/中医食养项为对应图片教程，图片中的食材/用量/做法/温馨提示即方案正文，不得裁切缩写），按运动干预 / 膳食干预 / 中医食养三大类分组，每类 1～2 项、总数不超过 6 项。素材缺失明确标记"素材待补齐"，禁止用其他图片替代。安全提示、禁忌与适宜人群信息在医生审核界面**和**患者报告页同时醒目展示，不能只在一端出现。
6. **评估报告生成不经医生前置审核**（2026-07-14 与用户确认的产品口径）：评估标签/得分是纯函数确定性计算，患者问答全部答完即自动触发评分并生成报告，患者可直接在大屏看到，医生**不需要**先确认评估结果这一步才能让患者看到。但**最终干预方案仍必须经医生审核（删/留/调整）**——患者看到的是未经审核的候选草案（`InterventionPlan.status = draft`），页面必须清楚标注"初步方案·医生确认中"，医生确认后（`status = confirmed`）患者端才会切换显示"医生已确认"。**Demo 补充（2026-07-20 拍板）**：测量题（BMI/腹围/小腿围）/观察题（舌象等）缺口不再阻断患者自助出报告——按 `deferClinical` 豁免计分、报告标注"部分计分"（见"已知的坑"条目与 `src/lib/scoring` 的 `partitionMissing`）；若有普通问答题"待人工确认"答案未补录，问答完成后评分仍会失败并回退 `in_progress`，患者端显示"需要医生协助"，走医生端 `CollectForm` 补录 + 手动点"完成采集"这条老路径（M2 起就有，未改变）。
...
患者原始语音与健康信息
  ...
  → 《评估-干预标签积分规则表.xlsx》（每标签对各干预项匹配分 0-3 → 累加总分 → 每类排序取前 2）
  → 候选干预集合（连同评估报告一起展示给患者，标注"初步方案"；含积分来源明细，医生可逐项下钻）
  → 素材教程展示（运动 M01-M12 视频/文字要点、膳食 D01-D10 图片、中医食养 C01-C08 图片，三大类分组）
  → 医生审核（保留/删除/同类替换，操作人+时间+原因留痕）→ 最终干预方案（患者端标注同步切换为"医生已确认"）
```
...
| `../需求更新说明_V2.0_20260719.md` | V2.0 功能需求补充（评估交互/补充评估/积分推荐/教程展示，架构依据） |
...
### 干预措施全集（V2.0：30 项，三大类；编码为素材关联唯一稳定标识）

- 运动干预 M01-M12（12 项，视频教程 + 文字要点）：名称以《12种运动干预.docx》为准
- 膳食干预 D01-D10（10 项，图文教程）：与 `膳食干预图片/` 按名称一一对应
- 中医食养干预 C01-C08（8 项，图文教程）：与 `中医食养干预图片/` 按名称一一对应

V1 的 15 个干预标签（八段锦、滋阴食养等）保留为历史资料，不再参与自动推荐。
...
├── docs/source/              # 医学源文件（只读）：量表/积分规则表/运动 docx/图片素材 + V1 历史资料
├── scripts/                  # convert-rules：源文件 → data/*.json 转换 + 全量校验（含积分 510 条复算）
├── data/                     # scales.json / intervention-scoring.json（V2 推荐唯一数据源）/ V1 历史 json
├── prisma/                   # schema + SQLite 数据库文件（.gitignore）
├── public/interventions/     # 干预素材：D/C 图片（convert-rules 拷贝）+ videos/（M01-M12 视频待补）
├── storage/audio-cache/      # TTS 音频缓存与录音文件（.gitignore）
├── src/components/           # 医患两端共用组件（intervention-media：视频/图片教程卡）
├── src/lib/scoring/          # 评分引擎：纯函数，无 IO，100% 测试覆盖
├── src/lib/recommend/        # 推荐引擎（V2.0 积分排名）：累加→每类排序取前2→附积分明细，纯函数
...
├── src/app/patient/          # 患者端大屏：数字医生、字幕、大字体大按钮、报告页补充评估入口
├── src/app/api/              # asr / tts / session 等 Route Handlers（密钥只在服务端）
└── tests/                    # vitest：引擎全量用例 + PII 过滤用例 + e2e（playwright）
```
