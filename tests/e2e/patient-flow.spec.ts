...
test("患者自助答完 FRAIL+跌倒后自动生成报告，医生端候选方案并行可审核", async ({ page }) => {
  ...
  // 走"手动选择作答"路径：这条用例验证的是按钮驱动的问答闭环，语音自动模式另有专门的 e2e 覆盖
  ...
  await page.getByRole("button", { name: "查看我的评估报告 →" }).click();
  ...
  await expect(page.getByRole("heading", { name: "八段锦", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "均衡膳食维持", exact: true })).toBeVisible();
  // 未确认的问询开始入口不应再出现（报告态与问询态互斥）
  await expect(page.getByRole("button", { name: "👆 手动选择作答" })).toHaveCount(0);
